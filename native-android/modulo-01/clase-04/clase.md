...
**Concepto**\
Existen fallos frecuentes al implementar Navigation Compose.
Identificarlos mejora la calidad de la app.

**Caso de uso**\
Un equipo que sufre problemas como rutas duplicadas, pérdida de estado o
ViewModels acoplados al `NavController`.
...
**Ejemplo completo**
  ...
  -------------------------- --------------------------------------------
  Pantallas duplicadas al    Usar `launchSingleTop = true`
  navegar                    

  Estado perdido al cambiar  Configurar `saveState` + `restoreState`
  de tab                     

  Rutas mal escritas         Centralizar en `sealed class` o rutas
  (strings mágicos)          type-safe

  ViewModel usando           Emitir eventos de navegación desde VM y
  `NavController`            manejarlos en la UI

  `popUpTo` borrando rutas   Validar qué ruta usar como `popUpTo` antes
  equivocadas                de navegar
  -----------------------------------------------------------------------
...
sealed class AppRoute(val route: String) {
    ...
    object Detail : AppRoute("detail/{id}") {
